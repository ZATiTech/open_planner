...
void opendrive::parser::RoadLinkParser::Parse(const pugi::xml_node &xmlNode, opendrive::RoadLink &out_road_link)
{
  ...
  if (predecessorNode)
  {
    out_road_link.predecessor = std::make_unique<opendrive::RoadLinkInformation>();
    parser.ParseLink(predecessorNode, out_road_link.predecessor.get());
  }

  if (successorNode)
  {
    out_road_link.successor = std::make_unique<opendrive::RoadLinkInformation>();
